...
public:
    ...
    // `location` is the folder the child is in; `here` are the names in it.
    static Reply notFound(const QString& name, const World& world, const QString& location,
                          const QStringList& here);
    static Reply isAPlace(const QString& name);
    static Reply isAThingToOpen(const QString& name);
    static Reply isANote(const QString& name);
