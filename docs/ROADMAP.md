...
| **D6** | Display manager and greeter (DESIGN §4.3 describes the login screen, not the component) | **SDDM with a Cairn QML theme remains proposed.** Since ADR-0002 the whole first-party layer is QML and SDDM themes are QML, so this would make the login screen a theme, not a program. `HideUsers` hides Guardians; a PAM rule grants passwordless login to `cairn-l1`/`cairn-l2` members. Verified 2026-09-04 on Fedora 44 KDE and 2026-09-06 on stock Bazzite KDE `44.20260902` in the VM: both ship Plasma Login Manager (`plasmalogin.service`), with sddm and greetd not installed. So the SDDM route means provisioning swaps the display manager, which the D6 ADR must make explicit. Alternative: greetd with a custom greeter, only if SDDM cannot do the avatar-tile login cleanly. Prototype in P0-3. | P0-3 |
