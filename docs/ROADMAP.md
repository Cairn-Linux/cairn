...
| **D2** | Launcher technology (DESIGN §14 Q1) | Prototype in **web tech inside an embedded WebKitGTK view** (Python + GTK4 + WebKitGTK 6, JS↔Python bridge via script messages). Fastest iteration on the visual design; a ~200-line host. **Measure idle RSS on the 2 GB tier in P0-5.** If it exceeds ~300 MB or feels slow on 2015 integrated graphics, switch to Qt/QML, which also aligns with Plasma and GCompris. Electron is ruled out. | P0-5 |
...
| **D6** | Display manager and greeter (DESIGN §4.3 describes the login screen, not the component) | **greetd + a first-party greeter** built with the same technology as the launcher, so the login screen is part of the frame (§6.3). PAM grants passwordless login to `cairn-l1`/`cairn-l2` members; Guardian accounts are hidden and password-protected. Alternative: SDDM with a custom QML theme and `HideUsers`. Prototype greetd in P0-3; fall back to SDDM if Plasma misbehaves under it. | P0-3 |
...
| **D9** | Name of the restricted shell | Needs a real name before Phase 1 packaging. Proposals welcome; constraint is that the child types it never — they see a "Terminal" tile — so it's a package name, not a brand. | P1-2 |
...
|---|---|---|
| Web launcher too heavy for the 2 GB tier | RSS measurement in P0-5 | Switch to Qt/QML before Phase 1 packaging (D2). |
